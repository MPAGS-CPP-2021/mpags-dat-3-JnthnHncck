...
int main(int argc, char* argv[])
{
    ...
    ProgramSettings s{false, false, "", "", "", false};

    // Process command line arguments
    const bool cmdLineStatus{
        processCommandLine(cmdLineArgs, s)};
    ...
    if (!s.inputFile.empty()) {
        // Open the file and check that we can read from it
        std::ifstream inputStream{s.inputFile};
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << s.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    // This class will turn a string or int input into a size_t
    CaesarCipher c{s.cipherKey};

    std::size_t caesarKey = c.key;
    

    // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
    std::string outputText{runCaesarCipher(inputText, caesarKey, s.encrypt)};

    // Output the encrypted/decrypted text to stdout/file
    if (!s.outputFile.empty()) {
        // Open the file and check that we can write to it
        std::ofstream outputStream{s.outputFile};
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << s.outputFile << "'" << std::endl;
            return 1;
        }
